...
df = pd.read_csv('wholesale-trade.csv')
st.dataframe(df)
...
with col1:
    ...
    sns.distplot(df['Data_value'], kde = False)
    st.pyplot(fig1)
with col2:
...
fig = plt.figure(figsize = (15,8))
sns.countplot(data = df, x = 'Magnitude')
st.pyplot(fig)

st.subheader('5.3 Box-Plot')
fig = plt.figure(figsize = (15,8))
sns.boxplot(data = df, x = 'Data_value', y = 'Magnitude')
st.pyplot(fig)

st.subheader('5.4 Violin-Plot')
fig = plt.figure(figsize = (15,8))
sns.violinplot(data = df, x = 'Data_value', y = 'Magnitude')
st.pyplot(fig)